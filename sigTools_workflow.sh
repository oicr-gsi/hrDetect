...
#GRCH38_ALLDIFFICULTREGIONS_ROOT=/.mounts/labs/gsi/modulator/sw/data/grch38-alldifficultregions-3.0/GRCh38_alldifficultregions.bed
...
bcftools query -f "%CHROM\t%POS\t%INFO/END\t%FILTER\t%ALT\t%INFO/CIPOS\t%INFO/CIEND\t[%DR\t]\t[%DV\t]\t[%RR\t]\t[%RV\t]\n" ${SV_file} | awk '$5 !~ ":" {print}' | awk '$4 ~ "PASS" {print}' | awk -v VAF=$VAF '($10+$14)/($8+$10+$12+$14) > VAF {print}' | awk -v sample_t=${sampleRoot}  'split($6,a,",") split($7,b,",") {print $1"\t"$2+a[1]-1"\t"$2+a[2]"\t"$1"\t"$3+b[1]-1"\t"$2+b[2]"\t"sample_t"\t"$5} ' | sed 's/<//g; s/>//g'  >>${sampleRoot}.somatic.delly.merged.reformat.bedpe

##task No2 make seperate .vcf files for SNVs and indels 

bcftools filter -i "(FORMAT/AD[0:1])/(FORMAT/AD[0:0]+FORMAT/AD[0:1]) >= ${VAF}" ${SNV_file} >${sampleRoot}.filter.deduped.realigned.recalibrated.mutect2.MAF15.vcf

for varType in SNP INDEL
do

	gatk SelectVariants -R ${HG38_ROOT}/hg38_random.fa  --exclude-intervals ${GRCH38_ALLDIFFICULTREGIONS_ROOT}/GRCh38_alldifficultregions.bed -V ${sampleRoot}.filter.deduped.realigned.recalibrated.mutect2.MAF15.vcf  --select-type-to-include ${varType} -O ${sampleRoot}.filter.deduped.realigned.recalibrated.mutect2.MAF15.${varType}.vcf
