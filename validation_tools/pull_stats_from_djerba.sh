...
zcat ${SNV_maf_file} |  awk '$1 ~ "BRCA" {print}' | sed 's/\t\t\t/\t.\t/g;' | awk '$9 !~ "'\''" {print}' | awk '$9 !~ "Intron" {print}' | awk '$9 !~ "Silent" {print}' | grep 'pathogenic' | awk -v study="$study" -v sampleRoot="$sampleRoot" ' {print $1"\t"$9"\t"$30/$29"\t"sampleRoot"\t"study}' |  awk '$3 > 0.1 {print}'  >>BRCA_mutations.${study}.2.txt
cat BRCA_mutations.${study}.2.txt
echo -e "\n"

done < ${wrkdir}/samples.${study}.2.txt
...
#do
VAF=05
awk -v VAF="$VAF" -v study="$study" -v sampleRoot="$sampleRoot" '{print study"\t"sampleRoot"\t"VAF"\t"$1"\t"$2}' ${sampleRoot}.sigtools.hrd.txt >>sigTools.MAFtest1.txt

#done
done < ${wrkdir}/samples.${study}.2.txt
done

awk -v VAF=05 -v study=PASS01 -v sampleRoot=PANX_1312 '{print study"\t"sampleRoot"\t"VAF"\t"$1"\t"$2}' PANX_1312.sigtools.hrd.txt >>sigTools.MAFtest1.txt

VAF=05

for study in TGL62 #PASS01 #
do
while read sampleRoot
do
echo -e "${study}\t${sampleRoot}" >>sigTools.samples.txt
#awk  -v study="$study" -v sampleRoot="$sampleRoot" '{print study"\t"sampleRoot}' ${sampleRoot}.HRDsummary.txt | uniq >>sigTools.samples.txt
done < ${wrkdir}/samples.${study}.txt
done
